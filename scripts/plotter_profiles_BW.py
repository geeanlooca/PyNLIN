...
for config in configs:
    ...
    for idx, power_dBm in enumerate(power_dBm_list):
        ...
        for pp in range(num_CO):
            ax_= ax.plot(z_max, watt2dBm(pump_solution_co[:, pp]), color=cm.inferno(0.95-(0.9*pp/(num_CO))))
        plt.xlabel("z [km]")
        ...
        #Get cmap values
        cmap_vals = cm.inferno([item/num_co for item in range(num_CO)])
        ...
        for pp in range(num_CNT):
            ax_= ax.plot(z_max, watt2dBm(pump_solution_cnt[:, pp]), color=cm.inferno(0.95-(0.9*pp/(num_CNT))))
        plt.xlabel("z [km]")
        plt.ylabel("Pump power [dBm]")
        plt.grid("on")
        plt.minorticks_on()
        ...
        #Get cmap values
        cmap_vals = cm.inferno([item/num_CNT for item in range(num_CNT)])
        ...
        for pp in range(num_bi):
            ax_= ax.plot(z_max, watt2dBm(pump_solution_bi[:, pp]), color=cm.inferno(0.95-(0.9*pp/(num_bi))))
        plt.xlabel("z [km]")
        plt.ylabel("Pump power [dBm]")
        plt.grid("on")
        plt.minorticks_on()
        ...
        #Get cmap values
        cmap_vals = cm.inferno([item/num_bi for item in range(num_bi)])
