...
for idx, power_dBm in enumerate(power_dBm_list):
    ...
    if True:
        ...
        ax.plot(z*1e-3, fB_co_1(z),color='black', linestyle='dashed',label = "CO",  linewidth = 3, zorder=-1)        
        ax.plot(z*1e-3, fB_cnt_1(z), color='black',linestyle='dotted',label = "CNT",  linewidth = 3, zorder=-1)
        ax.plot(z*1e-3, fB_bi_1(z), color='black', linestyle='dashdot',label = "BI",  linewidth = 3, zorder=-1)
...
##
        ...
        c = cm.viridis(np.linspace(0.1, 0.9, 4),1)
        # format(wdm.frequency_grid()[select_idx[1]]*1e-12, ".1f")
        ax.plot(z*1e-3, fB_co_2(z), color='black', linestyle='dashed',label = "CO", linewidth = 3, zorder=-1)
        ax.plot(z*1e-3, fB_cnt_2(z), color='black', linestyle='dotted',label = "CNT", linewidth = 3, zorder=-1)
        ax.plot(z*1e-3, fB_bi_2(z), color='black', linestyle='dashdot',label = "BI", linewidth = 3, zorder=-1)
...
##
    ...
    if True:
        ...
        for i, m_ in enumerate(m[5:-5]):
            ...
            ax1.plot(z*1e-3, fB_co(z), color='black', linestyle='dashed', dashes=(6, 2))
            ax1.axvline(locs[i] * 1e-3,color='grey', linestyle='dotted')
            ax1.axhline(0.5, color="grey", linestyle="dashdot", dashes=(6, 3))
        ...
        plt.subplots_adjust(left = 0.1, bottom=0.2, wspace=0.0, hspace=0.0, right = 9.8/10, top=9.9/10)
        ...
        plt.semilogy(m, np.abs(X0mm_co), marker='o', markersize = 10, color='black', label="CO")
        plt.semilogy(m, np.abs(X0mm_cnt), marker='x', markersize = 10, color='black',label="CNT")
        plt.semilogy(m, np.abs(X0mm_bi), marker='*', markersize = 10, color='black', label="BI")
...
ax1.plot(interfering_list, [1e-3*quad(fB_intx, 0, fiber_length)[0] for fB_intx in fB_co], marker='o', markersize = 7, color='black',  label="coprop.")
ax2.plot(interfering_list,  [1e-3*quad(fB_intx, 0, fiber_length)[0] for fB_intx in fB_cnt], marker='x', markersize = 7,color='black', label="counterprop.")
ax3.plot(interfering_list,  [1e-3*quad(fB_intx, 0, fiber_length)[0] for fB_intx in fB_bi], marker='*', markersize = 7,color='black', label="bidir.")
