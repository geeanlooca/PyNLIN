...
for fiber_length in fiber_lengths:
		...
		for pow_idx, power_dBm in enumerate(power_dBm_list):
				...
				for coi_idx, coi in enumerate(coi_list):
						...
						Delta_theta_2_co[coi_idx, pow_idx] =   16/9 * fiber.gamma**2 * constellation_variance * np.abs(X_co[coi_idx, pow_idx])
						Delta_theta_2_ct[coi_idx, pow_idx] =   16/9 * fiber.gamma**2 * constellation_variance * np.abs(X_ct[coi_idx, pow_idx])
						Delta_theta_2_bi[coi_idx, pow_idx] =   16/9 * fiber.gamma**2 * constellation_variance * np.abs(X_bi[coi_idx, pow_idx])
						Delta_theta_2_none[coi_idx, pow_idx] = 16/9 * fiber.gamma**2 * constellation_variance * np.abs(X_none[coi_idx, pow_idx])
...
## PLOTTING
		...
		if 'NLIN_vs_power' in plot_selection:
				...
				for scan in range(len(coi_selection)):
						ax1.plot(power_dBm_list, 10 * np.log10(Delta_theta_2_co[coi_selection_idx[scan], :] * P_A) , marker=markers[scan],
										markersize=10, color='green', label="ch." + str(coi_selection[scan]) + " co.")
						ax2.plot(power_dBm_list, 10 * np.log10(Delta_theta_2_ct[coi_selection_idx[scan], :] * P_A) , marker=markers[scan],
										markersize=10, color='blue', label="ch." + str(coi_selection[scan]) + " count.")
						ax3.plot(power_dBm_list, 10 * np.log10(Delta_theta_2_bi[coi_selection_idx[scan], :] * P_A), marker=markers[scan],
		...
		if 'NLIN_vs_power' in plot_selection:
				...
				plt.plot(show=True)
				for scan in range(len(coi_selection)):
						ax1.plot(power_dBm_list, 10 * np.log10(R_co[coi_selection_idx[scan], :] * P_A) , marker=markers[scan],
										markersize=10, color='green', label="ch." + str(coi_selection[scan]) + " co.")
						ax2.plot(power_dBm_list, 10 * np.log10(R_ct[coi_selection_idx[scan], :] * P_A) , marker=markers[scan],
										markersize=10, color='blue', label="ch." + str(coi_selection[scan]) + " count.")
						ax3.plot(power_dBm_list, 10 * np.log10(R_bi[coi_selection_idx[scan], :] * P_A), marker=markers[scan],
										markersize=10, color='orange', label="ch." + str(coi_selection[scan] + 1))
				...
				ax4.yaxis.tick_right()
				plt.subplots_adjust(wspace=0.0, hspace=0, right=8.5 / 10, top=9.9 / 10)
				fig_power.savefig(plot_save_path + "SRSN_vs_power.pdf")
		...
		if 'NLIN_and_ASE_and_SRSN_vs_power' in plot_selection:
				...
				plt.plot(show=True)
				axis_num = 0
				# plt.plot(power_dBm_list, 30 + 10 * np.log10(np.average([P_A * Delta_theta_2_co[coi_idx, :] for coi_idx in coi_selection_idx_average], axis=axis_num)), marker=markers[0], markersize=10, color='green', label="NLIN")
				# plt.plot(power_dBm_list, 30 + 10 * np.log10(np.average([P_A * Delta_theta_2_ct[coi_idx, :] for coi_idx in coi_selection_idx_average], axis=axis_num)), marker=markers[0], markersize=10, color='blue')
				# plt.plot(power_dBm_list, 30 + 10 * np.log10(np.average([P_A * Delta_theta_2_bi[coi_idx, :] for coi_idx in coi_selection_idx_average], axis=axis_num)), marker=markers[0], markersize=10, color='orange')

				print(R_ct[0, 0])
				print(Delta_theta_2_ct[0, 0])
				plt.plot(power_dBm_list, 10 * np.log10(np.average([(R_co[coi_idx, :] / Delta_theta_2_co[coi_idx, :]) for coi_idx in coi_selection_idx_average], axis=axis_num)), marker=markers[2], markersize=10, color='green', label="SRSN")
				plt.plot(power_dBm_list, 10 * np.log10(np.average([(R_ct[coi_idx, :] / Delta_theta_2_ct[coi_idx, :]) for coi_idx in coi_selection_idx_average], axis=axis_num)), marker=markers[2], markersize=10, color='blue')
				plt.plot(power_dBm_list, 10 * np.log10(np.average([(R_bi[coi_idx, :] / Delta_theta_2_bi[coi_idx, :]) for coi_idx in coi_selection_idx_average], axis=axis_num)), marker=markers[2], markersize=10, color='orange')
				...
				plt.xlabel(r"Signal input power [dBm]")
				plt.minorticks_on()
				plt.ylabel(r"Total noise to NLIN ratio [dB]")
				plt.grid()
				plt.minorticks_on()
				#plt.ylim([-90, 0])
		...
		if 'OSNR_vs_power' in plot_selection:
				...
				#plt.ylim([20, 50])
				plt.ylabel("OSNR [dB]")
				plt.xlabel("Signal input power [dBm]")
				plt.legend()
				plt.subplots_adjust(wspace=0.0, hspace=0, right=8.5 / 10, top=9.9 / 10)
		...
		if 'OSNR_ASE_vs_wavelength' in plot_selection:
				...
				plt.plot(show=True)
				plt.plot(wavelength_list, 10 * np.log10(power_at_receiver_co[:, pow_idx] / ase_ct[:, pow_idx]), marker='x', markersize=15, ls='dashed', color='blue', label='CT')
		...
		if 'NLIN_vs_wavelength_multiple' in plot_selection:
				fig_NLIN_channel, ((ax1)) = plt.subplots(nrows=1, ncols=1, sharex=True, figsize=(plot_width, 10))
				plt.plot(show=True)
				tmp = pow_idx
				plt.plot(wavelength_list, 10 * np.log10(P_A[pow_idx] * Delta_theta_2_co[:, pow_idx]) + 30, marker='x', markersize=15, ls='dotted', color='green', label='CO')
				...
				plt.legend(bbox_to_anchor=(1, 0.6), loc='center right')
				plt.tight_layout()
				fig_NLIN_channel.savefig(plot_save_path + "NLIN_vs_wavelength_multiple.pdf")
		...
		if 'Pumps_all' in plot_selection:
			...
			for pp in range(3):
				plt.plot(power_dBm_list, all_co_pumps[:, pp], marker=markers[0],
										markersize=5, linestyle='dotted', color='green')
			plt.plot(power_dBm_list, all_co_pumps[:, pp+1], marker=markers[0],
										markersize=5, linestyle='dotted', color='green',label='CO')
			for pp in range(3):
				plt.plot(power_dBm_list, all_ct_pumps[:, pp],linestyle='dashed', marker=markers[1],
										markersize=6, color='blue')

			plt.plot(power_dBm_list, all_ct_pumps[:, pp+1],linestyle='dashed', marker=markers[1],
										markersize=6, color='blue', label='CT')
			for pp in range(5):
				plt.plot(power_dBm_list, all_bi_pumps[:, pp],  marker=markers[2],
										markersize=6, color='orange' )
			plt.plot(power_dBm_list, all_bi_pumps[:, pp+1],  marker=markers[2],
										markersize=6, color='orange', label='BI' )
			...
			#plt.ylim([20, 50])
			plt.ylabel(r"Pump input power [dBm]")
			plt.xlabel(r"Signal input power [dBm]")
			#plt.ylim([17.0, 35.0])
			plt.grid(which="both")
			plt.tight_layout()
